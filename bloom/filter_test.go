...
import (
	...
	"github.com/beyondcoin-project/byndutil/bloom"
)
...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := byndutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(byndutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := byndutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := byndutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
