...
func (cs *CoinSet) TotalValue() (value byndutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue byndutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue byndutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount byndutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue byndutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue byndutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue byndutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue byndutil.Amount, coins []Coin) (Coins, error) {
...
// byndutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() byndutil.Amount {
	return byndutil.Amount(c.txOut().Value)
}
